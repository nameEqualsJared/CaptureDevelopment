...
searchBar.onblur = function () {
    ...
    if (searchText !== "") {
        ...
        let searchConditions = searchText.split(" "); // will be an array like ["MDN", "#web"] 
        // notice that we can mix tags and titles just fine

        // build the array of snips that meet each condition, and store all of them in an array. 
        // eg: if searchConditions = ["MDN", "#web"], then 
        // conditionsMatchArray will = [[array of ids of snips with MDN in the title], [array of ids of snips with the #web tag]].
        conditionsMatchArray = [];
        for (let condition of searchConditions) {
            if (condition[0] === "#") {

                dbForTags.get(condition.slice(1)).then(function (result) {
                    conditionsMatchArray.push(result.snipsWithThisTag);
                });

            } else {

                conditionsMatchArray.push(snipsMeetingThisTitleCondition(condition));

            }
        }

        for (let elem of conditionsMatchArray) {
            console.log(elem);
        }
        //----------------needs fixing------

        //compute the intersection of all the arrays in the conditionsMatchArray. This represents the snips that meet all of the search conditions.

        //console.log([a, b].reduce(intersection));

        // for (let id of snipsMatchingAllConditions) {
        // 	renderSnipToHTML
        // }
...
function intersection(arr1, arr2) {
    // given two arrays, function returns their intersection (with no duplicates)
    // i.e., intersection([5, 5], [1, 2, 5]) = [5]
    let res = arr1.filter(value => -1 !== arr2.indexOf(value));
    return [...new Set(res)];
}

function snipsMeetingThisTitleCondition(condition) {
	/*
		condition: a string specifiying the title search condition (from the search bar) 
				eg, condition = "MDN"
		
		Method returns an array of the ids of the snips meeting the title condition.
		I.e., if the condition is "MDN", this function returns an array of ids of all the snips with MDN in the title.
	*/
    ...
    dbForSnips.allDocs({ include_docs: true, descending: true }, function (err, doc) {
        if (err) {
            ...
            for (let entry of doc.rows) {
                ...
                if (snip.title.includes(condition)) {
                    result.push(snip._id);
                }
