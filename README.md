...
One last thing to note: the tag system. Filtering by a tag works by clicking on a tag in the sidebar, which toggles the tag on and shows owly the snips with that tag, [like so](https://i.imgur.com/1aA49xQ.png). You can also click on multiple tags to show a union of snips: i.e., if the prog and css tags are on, then it will show both the snips with the prog tag and the snips with the css tag, [like this](https://i.imgur.com/SIqCgqI.png).
